function DataSet = dropcspm_ProgramType(app,varargin)
...
switch ProcessType
    case 1 % generates labels
        NoOfEvents = 17;
        DataSet.draq_d.nEventTypes=NoOfEvents;
        DataSet.draq_d.nEvPerType=zeros(1,NoOfEvents);
        DataSet.draq_d.eventlabels=cell(1,NoOfEvents);
                ...
                if exist('t_start','var')
                    ...
                    if sum(shiftdata(t_start:t_start+6*DataSet.draq_p.ActualRate)==8)>0.05*DataSet.draq_p.ActualRate
                        ...
                        if DataSet.generate_dio_bits==1
                            DataSet.dio_bits(:,trialNo)=DataSet.dio_bits(:,trialNo)-1;
                            shiftvec=bitshift( bitand(uint16(DataSet.dio_bits(:,trialNo)),248), -2);
                            fv=shiftvec==6;
                            DataSet.dio_bits(:,trialNo)=DataSet.dio_bits(:,trialNo)+fv;
                        end
                    ...
                    if sum(shiftdata(t_start:t_start+6*DataSet.draq_p.ActualRate)==12)>0.05*DataSet.draq_p.ActualRate
                        ...
                        if DataSet.generate_dio_bits==1
                            DataSet.dio_bits(:,trialNo)=DataSet.DataSet.dio_bits(:,trialNo)-1;
                        end
