function LaserTriggered_ProgramType(app,varargin)
...
switch ProcessType
    ...
    case 3 % create events
        data = varargin{2};
        trialNo = varargin{3};
        if sum(data(:,21)>(app.drta_handles.draq_d.min_laser+(app.drta_handles.draq_d.max_laser-app.drta_handles.draq_d.min_laser)/2))==0
            ...
            k=find(data(ceil(2.5*app.drta_handles.draq_p.ActualRate):end,21)>(app.drta_handles.draq_d.min_laser+(app.drta_handles.draq_d.max_laser-app.drta_handles.draq_d.min_laser)/2),1,'first');
