function handles = drta03_ExcludeBadLFP(app,handles)
%   This function excludes bad LFPs

if isfield(handles,'drtachoices')
    textUpdate =  sprintf('Vetting LFPs...');
...
parfor lfpno=1:noch
    if ~isfield(handles,'drtachoices')
        textUpdate = sprintf('Vetting LFP number %d\n',lfpno);
        fprintf(1,textUpdate);
    end
    handlespf=handles;
    for trNo=1:handlespf.draq_d.noTrials
        if handlespf.p.trial_allch_processed(trNo)==1
            ...
            data_this_trial=drtaNWB_GetTraceData(handlespf);
            data=[];
