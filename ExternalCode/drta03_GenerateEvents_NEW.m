...
for trialNo=1:TrialCount
    ...
    if trialNo==1
        ...
        ElectrodDataPerTrial = nan([size(data,1),size(data,2),TrialCount]);
        %need to add condition that if the data is >1 or it will be a nxm
        %for both analog and digital
        AnalogDataPerTrial = nan([size(data,1),size(data,2),TrialCount]);
        DigitalDataPerTrial = nan([size(data,1),size(data,2),TrialCount]);
    ...
    AnalogDataPerTrial(:,:,trialNo) = data;
    %need if statment for >1 channel
    DigitalDataPerTrial(:,:,trialNo) = data;
    ElectrodDataPerTrial(:,:,trialNo) = data;
