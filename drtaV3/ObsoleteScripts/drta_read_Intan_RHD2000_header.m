function [draq_p,draq_d]=drta_read_Intan_RHD2000_header(handles)
...
% d = dir([path '*.rhd']);
% file = d(end).name;
filename = handles.p.fullName;
which_protocol = handles.p.which_protocol;
tic;
...
ReadoutUpdate(handles.GUIs.drtaLoad,['Reading Intan Technologies RHD2000 Data File, Version ', ...
    num2str(data_file_main_version_number), num2str(data_file_secondary_version_number)]);
...
ReadoutUpdate(handles.GUIs.drtaLoad,[['Sample rate = ',num2str(sample_rate)], ...
    ['Lower bandwidth = ', num2str(actual_lower_bandwidth)], ...
    ['Upper_bandwidth = ', num2str(actual_upper_bandwidth)]]);
...
%     fprintf(1, 'Reading data from file...\n');
    ...
    for i=1:num_data_blocks
        ...
        if (fraction_done >= percent_done)
            ReadoutUpdate(handles.GUIs.drtaLoad,[percent_done; ' percent_done']);
